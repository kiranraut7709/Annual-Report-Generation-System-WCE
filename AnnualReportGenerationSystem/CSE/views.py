...
def phdDetails(request):
    if isAuthenticatedDepartment(request):
        user = request.user
        allPhd = PHD.objects.filter(user=user).values('phd_id', 'title', 'guide_name', 'registration_date', 'completion_date', 'status').annotate(dCount=Count('phd_id')).order_by("registration_date")
        args = {
            "allPhd": allPhd,
            "showAll": True
        }
...
def showAllPhdData(request):
    if isAuthenticatedDepartment(request):
        user = request.user
        allPhd = PHD.objects.filter(user=user).order_by("registration_date")
        args = {
            "allPhd": allPhd,
            "showAll": False
        }
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                ...
                phdData = PHD(user=request.user, phd_id=phd_id, department='Computer Science and Engineering', guide_name=guide_name, faculty_name=name,
                              registration_date=registration_date, title=title, status=True,
                              completion_date=completion_date, academic_year=academic_year)
            else:
                if PHD.objects.filter(phd_id=phd_id).filter(status=True).exists():
                    messages.info(request, "First add PHD entry of status Ongoing and then add PHD entry of status Completed!")
                    return redirect("CSE:addPHD")

                if PHD.objects.filter(phd_id=phd_id).exists():
                    messages.info(request, "Record Already Exist!")
                    return redirect("CSE:addPHD")

                phdData = PHD(user=request.user, phd_id=phd_id, department='Computer Science and Engineering', guide_name=guide_name, faculty_name=name,
                              registration_date=registration_date, title=title, status=False, academic_year=academic_year)
...
def updatePhdData(request, phd_id):
    academic_year = int(request.POST['academic_year'])
    ...
    name = request.user.first_name + " " + request.user.last_name
    if PHD.objects.filter(academic_year=academic_year-1).exists():
        phd = PHD.objects.filter(academic_year=academic_year-1).get(phd_id=phd_id)

        guide_name = phd.guide_name
        registration_date = phd.registration_date
        title = phd.title

        for i in range(toYear-academic_year):
            if PHD.objects.filter(academic_year=academic_year+i).filter(phd_id=phd_id).exists():
                messages.info(request, "Record Already Exist!")
                return redirect("CSE:phdDetails")

            phdData = PHD(user=request.user, phd_id=phd_id, department='Computer Science and Engineering', guide_name=guide_name,
                          faculty_name=name,
                          registration_date=registration_date, title=title, status=False, academic_year=academic_year+i)

            phdData.save()
    else:
        messages.info(request, "First add PHD entry of Academic Year " + str(academic_year-1) + "-" + str(academic_year) + " and then add " + str(academic_year) + "-" + str(toYear) + " Academic Year PHD Entry!")
    return redirect("CSE:phdDetails")


def phdDetailsDelete(request, phd_id):
    if isAuthenticatedDepartment(request):
        phdData = PHD.objects.filter(phd_id=phd_id)
        phdData.delete()
        return redirect("CSE:phdDetails")
    return HttpResponse("<h1>Page Not Found</h1>")
...
def studentData(request):
    if isAuthenticatedDepartment(request):
        ...
        allStudent = BTechStudentData.objects.filter(academic_year__gte=academic_year, academic_year__lte=int(toYear)-1).order_by('PRN')
...
def guidePhdDetails(request):
    if isAuthenticatedDepartment(request):
        user = request.user
        guideAllPhd = GuideforPHD.objects.filter(user=user).values('phd_id', 'student_name', 'title_thesis', 'guide_name', 'comment_on_thesis', 'type_of_phd', 'registration_date', 'completion_date', 'status').annotate(dCount=Count('phd_id')).order_by("registration_date")
        args = {
            "guideAllPhd": guideAllPhd,
            "showAll": True
        }
...
def showAllGuidePhdData(request):
    if isAuthenticatedDepartment(request):
        user = request.user
        guideAllPhd = GuideforPHD.objects.filter(user=user).order_by("registration_date")
        args = {
            "guideAllPhd": guideAllPhd,
            "showAll": False
        }
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                ...
                guidePhdData = GuideforPHD(user=request.user, phd_id=phd_id, student_name=student_name,
                                           ...
                                           status=True, completion_date=completion_date, academic_year=academic_year)

            else:
                if GuideforPHD.objects.filter(phd_id=phd_id).filter(status=True).exists():
                    messages.info(request, "First add PHD entry of status Ongoing and then add PHD entry of status Completed!")
                    return redirect("CSE:addGuidePHD")

                if GuideforPHD.objects.filter(phd_id=phd_id).exists():
                    messages.info(request, "Record Already Exist!")
                    return redirect("CSE:addGuidePHD")
                ...
                guidePhdData = GuideforPHD(user=request.user, phd_id=phd_id, student_name=student_name,
                                           registration_date=registration_date, title_thesis=title_thesis,
                                           guide_name=guide_name, comment_on_thesis=comment_on_thesis, type_of_phd=type_of_phd,
                                           status=False, academic_year=academic_year)
...
def updateGuidePhdData(request, phd_id):
    academic_year = int(request.POST['academic_year'])
    toYear = int(request.POST['toYear'])
    ...
    if GuideforPHD.objects.filter(academic_year=academic_year-1).exists():
        phd = GuideforPHD.objects.filter(academic_year=academic_year-1).get(phd_id=phd_id)

        student_name = phd.student_name
        registration_date = phd.registration_date
        title_thesis = phd.title_thesis
        guide_name = phd.guide_name
        comment_on_thesis = phd.comment_on_thesis
        type_of_phd = phd.type_of_phd

        for i in range(toYear-academic_year):
            if GuideforPHD.objects.filter(academic_year=academic_year+i).filter(phd_id=phd_id).exists():
                messages.info(request, "Record Already Exist!")
                return redirect("CSE:guidePhdDetails")
            ...
            guidePhdData = GuideforPHD(user=request.user, phd_id=phd_id, student_name=student_name,
                                       registration_date=registration_date, title_thesis=title_thesis,
                                       guide_name=guide_name, comment_on_thesis=comment_on_thesis, type_of_phd=type_of_phd,
                                       status=False, academic_year=academic_year+i)
...
def guidePhdDetailsDelete(request, id):
    if isAuthenticatedDepartment(request):
        guidePhdData = GuideforPHD.objects.filter(phd_id=id)
        guidePhdData.delete()
        return redirect("CSE:guidePhdDetails")
    return HttpResponse("<h1>Page Not Found</h1>")
